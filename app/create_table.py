...
class CreateTableResponse(BaseModel):
    success: bool
    table_name: str
    full_table_name: str   # dataset.<name>
    rows_loaded: int
    warnings: list[str]
...
def _build_ddl(table_name: str, columns: list[ColumnInput], warnings: list[str]) -> tuple[str, list[ColumnInput]]:
    """
...
def _create_and_load(
    ...
    """
    Opens a raw psycopg2 connection, acquires an advisory lock,
    creates the table, bulk-inserts rows, runs ANALYZE, commits.
    ...
    try:
        with conn.cursor() as cur:
            # Advisory lock — hash the table name to an integer
            ...
            cur.execute(
                ...
                WHERE table_schema = 'dataset' AND table_name = %s
                """,
                (table_name,),
            )
            exists = cur.fetchone()
            if exists and not overwrite:
                raise ValueError(
                    f"Table 'dataset.{table_name}' already exists. "
                    "Tick 'Overwrite if exists' and click Create table again to replace it."
                )
            if exists and overwrite:
                cur.execute(f'DROP TABLE dataset."{table_name}"')
            ...
            cur.execute(f'ANALYZE dataset."{table_name}"')
...
def _create_table_job(
    ...
    warnings: list[str] = []
    ddl, resolved_cols = _build_ddl(table_name, columns, warnings)
    ...
    # Create table + load
    rows_loaded = _create_and_load(ddl, table_name, col_names, rows, overwrite)
...
async def create_table(
    ...
    if not payload.overwrite:
        ...
        exists = await loop.run_in_executor(None, _table_exists, payload.table_name)
        if exists:
            raise HTTPException(
                status_code=409,
                detail=(
                    f"Table 'dataset.{payload.table_name}' already exists. "
                    "Tick 'Overwrite if exists' and click Create table again to replace it."
                ),
            )
...
def _table_exists(table_name: str) -> bool:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT 1 FROM information_schema.tables WHERE table_schema='dataset' AND table_name=%s",
                (table_name,),
            )
